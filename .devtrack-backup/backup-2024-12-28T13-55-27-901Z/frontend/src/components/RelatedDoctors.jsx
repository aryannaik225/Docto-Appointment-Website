...
const RelatedDoctors = ({docId,speciality}) => {
  ...
  useEffect(()=>{
    if(doctors.length > 0 && speciality){
      const doctorsData = doctors.filter((doc)=> doc.speciality === speciality && doc._id !== docId) // filtering all the doctors with the same speciality as the current doctor except the current doctor
